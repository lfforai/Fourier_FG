...
int main()
{
	...
	Mat Lena = imread("C:/Users/Administrator/Desktop/output/lena.jpg");
	...
	int imgWidth_des = floor(imgWidth_src * x_rato);
	int imgHeight_des = floor(imgHeight_src * y_rato);
	...
	//输出图像
	Mat dstImg1 = Mat::zeros(imgHeight_des, imgWidth_des, CV_8UC1);//自己编写的函数用
	Mat dstImg2 = Mat::zeros(imgHeight_des, imgWidth_des, CV_8UC1);//cuda用的

	uchar* pDstImgData1 = NULL;
	t = cudaMalloc(&pDstImgData1, imgHeight_des * imgWidth_des * sizeof(uchar));

	uchar* pDstImgData2 = NULL;
	t = cudaMalloc(&pDstImgData2, imgHeight_des * imgWidth_des * sizeof(uchar));

	dim3 block(8, 8);
	dim3 grid((imgWidth_des + block.x - 1) / block.x, (imgHeight_des + block.y - 1) / block.y);

	weightAddKerkel << <grid, block >> > (pDstImgData1, imgHeight_des, imgWidth_des, y_rato, x_rato);
	cudaDeviceSynchronize();
	weightAddKerkel_cuda << <grid, block >> > (pDstImgData2, imgHeight_des, imgWidth_des, y_rato, x_rato);
	cudaDeviceSynchronize();

	//从GPU拷贝输出数据到CPU
	t = cudaMemcpy(dstImg1.data, pDstImgData1, imgWidth_des * imgHeight_des * sizeof(uchar)*channels, cudaMemcpyDeviceToHost);
	t = cudaMemcpy(dstImg2.data, pDstImgData2, imgWidth_des * imgHeight_des * sizeof(uchar)*channels, cudaMemcpyDeviceToHost);
